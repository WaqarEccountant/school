    ...
    <div class="card">
        <div class="card-header row">
            ...
            <div class="col-2 text-right">
                <a href="{{route('teacher.exams.create')}}" class="btn btn-sm btn-success">New</a>
            </div>
        </div>
        <div class="card-body">
                    ...
                    @if (isset($records) && count($records))
                        ...
                        @foreach($records as $row)
                            <tr>
                                ...
                                <td>{{$row->type_name}}</td>
                                <td>
                                    <a rel="tooltip" class="btn btn-primary btn-sm btn-round btn-icon" title="view" href="{{route('teacher.exams.show',$row->id)}}">
                                        <i class="now-ui-icons gestures_tap-01"></i>
                                    </a>
                                    @if ($row->isOwner($user_id))
                                        <a rel="tooltip" class="btn btn-info btn-sm btn-round btn-icon" title="edit" href="{{route('teacher.exams.edit',$row->id)}}" >
                                        ...
                                        <button type="button" rel="tooltip" class="btn btn-danger btn-sm btn-round btn-icon" title="delete" data-toggle="modal" data-target="#deleteSubject"
                                            data-link="{{route('teacher.exams.destroy',$row->id)}}" onclick="deleteRow(this)">
