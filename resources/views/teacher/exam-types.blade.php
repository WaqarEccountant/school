...
@section('content')
    <div class="card">
        <div class="card-header row">
            <h3 class="col">Exam Types</h3>
            <div class="col-2 text-right">
                <button type="button" data-toggle="modal" data-target="#addModal" class="btn btn-sm btn-success">New</button>
            </div>
        </div>
        <div class="card-body">
                                ...
                                <td>
                                    @if ($row->isOwner($user_id))
                                        <button type="button" rel="tooltip" class="btn btn-info btn-sm btn-round btn-icon" data-toggle="modal" data-target="#editModal"
                                            data-name="{{$row->name}}" data-link="{{route('teacher.exam-types.update',$row->id)}}" onclick="editRow(this)">
                                        ...
                                        <button type="button" rel="tooltip" class="btn btn-danger btn-sm btn-round btn-icon" data-toggle="modal" data-target="#deleteModal"
                                            data-link="{{route('teacher.exam-types.destroy',$row->id)}}" onclick="deleteRow(this)">
    ...
    <div class="modal fade" id="addModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form method="post" action="{{route('teacher.exam-types.store')}}">
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form method="post" action="" id="delete-form">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="exampleModalLabel">Delete Exam Type</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
